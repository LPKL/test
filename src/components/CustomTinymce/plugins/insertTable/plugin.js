...
(function(domGlobals) {
  ...
  // table的toolbar按钮
  // TODO: tablemodel是table关联模型功能
  var defaultTableToolbar = 'tablemodel tableprops tabledelete | tableinsertrowbefore tableinsertrowafter tabledeleterow | tableinsertcolbefore tableinsertcolafter tabledeletecol'
  var rel_TableToolbar = 'tablemodel tableconfig tabledelete'
  var defaultStyles = {
  ...
  // 已关联模型的table，不显示行列操作，只保留模型关联和删除按钮
  var getRefModelToolbar = function(editor) {
  ...
  var extractAutoAttrDataFromTableElement = function(editor, elm) {
    ...
    var data = __assign({
      ...
      datamodeltype: dom.getAttrib(elm, 'data-model-type', ''),
      dataparams: dom.getAttrib(elm, 'data-param', ''),
      dataparamsmark: dom.getAttrib(elm, 'data-parammark', ''),
  ...
  var registerCommands = function(editor, actions, cellSelection, selections, clipboardRows) {
    ...
    editor.addCommand('mceTableModel', function() {
      // 模型关联dialog
      var table_elem = editor.dom.getParent(editor.selection.getStart(), 'table')
      if (!table_elem) {
        // 直接点击关联模型按钮，没有表格情况下，要创建一个基础表格
        InsertTable.insert(editor, 1, 1)
      }
      return editor.settings.get_modelitems_callback(editor, 'table_model_variable_type')
    })
    editor.addCommand('mceTableConfig', function() {
      // 模型设置的dialog(显示的数据个数)
      // return editor.settings.set_specialtable_count_callback(editor)
    })
  }
  ...
  var addButtons = function(editor, selectionTargets) {
    editor.ui.registry.addMenuButton('insertTableM', {
      tooltip: '插入可关联模型的表格',
      text: '表格',
      icon: 'insert-table',
      fetch: function(callback) {
        ...
        // menubar上，对于已关联模型的table，只显示模型关联模型按钮和删除表格按钮
        if (autodata.datamodel === '') {
          return callback('inserttable tablemodel tableprops deletetable | cell row column')
        }
        return callback('tablemodel tableconfig deletetable')
      }
    })
    ...
    // tablemodel 是table关联模型功能
    editor.ui.registry.addButton('tablemodel', {
      tooltip: '关联模型',
      onAction: cmd('mceTableModel'),
      icon: 'table-model',
      onSetup: selectionTargets.onSetupModel
    })
    editor.ui.registry.addButton('tableconfig', {
      tooltip: '表格设置',
      onAction: cmd('mceTableConfig'),
      icon: 'chart-data-count',
      onSetup: selectionTargets.onSetupModel
    })
  ...
  var addToolbars = function(editor) {
    ...
    var isTableModel = function(table) {
      var autodata = Helpers.extractAutoAttrDataFromTableElement(editor, table)
      if (autodata.datamodel !== '') {
        return editor.dom.is(table, 'table') && editor.getBody().contains(table)
      }
    ...
    // 关联过模型的table，只有关联模型和删除按钮
    editor.ui.registry.addContextToolbar('tableModel', {
      predicate: isTableModel,
      items: getRefModelToolbar(editor),
      scope: 'node',
      position: 'node'
    })
    editor.ui.registry.addContextToolbar('tableConfig', {
      predicate: isTableModel,
      items: getRefModelToolbar(editor),
      scope: 'node',
      position: 'node'
  ...
  var addMenuItems = function(editor, selectionTargets) {
    ...
    editor.ui.registry.addMenuItem('tablemodel', tableCorrelationModel) // 关联模型
    ...
    editor.ui.registry.addContextMenu('insertTableM', {
      update: function() {
  ...
  function Plugin$1() {
    global.add('insertTableM', Plugin)
  }
