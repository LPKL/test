/* eslint-disable */
import { postTemplateAttachment } from '@/views/report/components/editorCommonMethod.js'
...
(function (domGlobals) {
    ...
    var makeItems = function (info) {
      ...
      var imageList = info.imageList.map(function (items) {
        // TODO：每次获取列表的时候重新请求imagelist，解决本页面上传新图片后未能加在列表上的问题
        return {
          name: 'images',
          type: 'selectbox',
          label: '图片列表',
    ...
    var hasImageTitle = function (editor) {
      return editor.getParam('image_title', true, 'boolean');
    };
    ...
    var getBaseUrl = function (editor) {
      return editor.getParam('upload_base_url', '', 'string');
    };
    var Settings = {
    ...
    var collect = function (editor) {
      ...
      var baseUrl = Settings.getBaseUrl(editor);
        ...
        Utils.blobToDataUri(file).then(function (dataUrl) {
          ...
          uploader.upload(blobInfo).then(function (url) {
            api.setData({
              src: {
                value: url,
                meta: {'alt': blobInfo.filename()} // MSG: 将文件名赋值给alt，作为默认名称
              },
            });
            // MSG: 上传后，跳转到插入图片配置tab
            api.showTab('general');
            changeSrc(helpers, info, state, api);
            finalize();
          }).catch(function (err) {
    ...
    var submitHandler = function (editor) {
      return function (info) {
        return function (api, evt) {
          if (api.getData().fileinput.length != 0) {
            // 上传图片第二步，需要将图片信息上传
            var postdata = {
              'file_uuid': api.getData().src.value,
              'name': api.getData().alt,
              'type': 'img'
            }
            ...
            // 非新上传图片，imagelist中选择后直接插入面板
            // 不显示显示后台地址，在此处拼接图片的url////////TODO,返回的真实地址
            api.setData({
              src: {
                value: info.baseUrl + api.getData().src.value,
              }
            });
            var data = deepMerge(fromImageData(info.image), api.getData());
            editor.undoManager.transact(function () {
    ...
    var register$1 = function (editor) {
      editor.ui.registry.addToggleButton('uploadImage', {
        icon: 'image',
        text: 'Image',
      ...
      editor.ui.registry.addContextMenu('uploadImage', {
    ...
    function Plugin () {
      global$1.add('uploadImage', function (editor) {
