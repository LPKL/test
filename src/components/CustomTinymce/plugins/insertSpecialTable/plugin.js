...
(function(domGlobals) {
      ...
      return lookupTable(start).bind(function(startTable) {
        return lookupTable(finish).bind(function(finishTable) {
          if (eq(startTable, finishTable)) {
            ...
            var ancestorCells1 = ancestors$1(start, 'td,th', getIsRoot(finishTable))
            var startCell = ancestorCells1.length > 0 ? ancestorCells1[ancestorCells1.length - 1] : start
            return Option.some(Identified.create({
  ...
  var rel_TableToolbar = 'specialtable tabledelete'
  var defaultStyles = {
  ...
  // 特定table，不显示行列操作，只保留特定表格设置按钮和删除按钮
  var getRefSpecialTableToolbar = function(editor) {
    return editor.getParam('table_toolbar', rel_TableToolbar)
  }
  ...
  // 获得自定义属性
  var extractAutoAttrDataFromTableElement = function(editor, elm) {
    var dom = editor.dom
    var data = __assign({
      datatabletype: dom.getAttrib(elm, 'data-tabletype', ''),
      datamodel: dom.getAttrib(elm, 'data-model', ''),
      dataparams: dom.getAttrib(elm, 'data-param-obj', ''),
      datadevice: dom.getAttrib(elm, 'data-device', ''),
      datarecordcount: dom.getAttrib(elm, 'data-recordcount', ''),
      datatimerange: dom.getAttrib(elm, 'data-timerange', ''),
      datarated: dom.getAttrib(elm, 'data-rated', ''),
      datatablehead: dom.getAttrib(elm, 'data-tablehead', '')
    })
    return data
  }
  ...
  var Helpers = {
    ...
    extractAutoAttrDataFromTableElement: extractAutoAttrDataFromTableElement
  }
  ...
  var insert$1 = function(editor, columns, rows, special) {
    ...
    if (special) {
      set(table, 'data-tabletype', special)
    }
    var html = getOuter$2(table)
    // console.log('hhhtml', html)
  ...
  // var TableModelDialog = { open: openModelPage }
  ...
  var registerCommands = function(editor, actions, cellSelection, selections, clipboardRows) {
    ...
    // 特殊表格配置dialog
    editor.addCommand('mceSpecialTable', function() {
      // 模型关联dialog
      var table_elem = editor.dom.getParent(editor.selection.getStart(), 'table')
      if (!table_elem) {
        // 直接点击关联模型按钮，没有表格情况下，要创建一个基础表格
        InsertTable.insert(editor, 1, 1, 'special')
      }
      // 设置界面
      return editor.settings.set_specialtable_callback(editor)
    })
  }
  ...
  var getSelectionTargets = function(editor, selections) {
    ...
    var onSetupModel = function(api) {
      return api.setDisabled(false) // 一直显示关联模型按钮选项
    }
  ...
  var addButtons = function(editor, selectionTargets) {
    editor.ui.registry.addMenuButton('insertSpecialTable', {
      tooltip: 'Insert table',
      text: 'Table',
      icon: 'insert-table',
      fetch: function(callback) {
        var tableElm = editor.dom.getParent(editor.selection.getStart(), 'table')
        var autodata = Helpers.extractAutoAttrDataFromTableElement(editor, tableElm)
        // menubar上，对于特定table，只显示特定表格设置按钮和删除表格按钮
        if (autodata.datatabletype === '') {
          return callback('specialtable | inserttable tableprops deletetable | cell row column')
        }
        return callback('specialtable deletetable')
      }
    })
    ...
    // specialtable 是插入特定table
    editor.ui.registry.addButton('specialtable', {
      tooltip: 'Special Table Config',
      onAction: cmd('mceSpecialTable'),
      icon: 'table-model',
      onSetup: selectionTargets.onSetupModel
    })
  ...
  var addToolbars = function(editor) {
    var isTable = function(table) {
      var autodata = Helpers.extractAutoAttrDataFromTableElement(editor, table)
      if (autodata.datatabletype === '') {
        return editor.dom.is(table, 'table') && editor.getBody().contains(table)
      }
    }
    var isSpecialTable = function(table) {
      var autodata = Helpers.extractAutoAttrDataFromTableElement(editor, table)
      if (autodata.datatabletype !== '') {
        return editor.dom.is(table, 'table') && editor.getBody().contains(table)
      }
    ...
    // 特定table，只有特定表格设置按钮和删除按钮
    editor.ui.registry.addContextToolbar('specialTable', {
      predicate: isSpecialTable,
      items: getRefSpecialTableToolbar(editor),
      scope: 'node',
      position: 'node'
    })
  }
  ...
  var addMenuItems = function(editor, selectionTargets) {
    ...
    // DONE: 插入表，弹出配置信息，
    // 1、选择表类型（监测参数、个性化阈值、相关性、趋势、健康度、负荷分布、发电量）
    // 2、根据表类型，提供下一步选择，主要是确定表头行，即每列显示什么信息（监测参数：模型-参数选择；个性化阈值：变量名称，个性化阈值。。。）提供一个自定义列
    // 3、列确定后，提供下一步选择，主要是确定每行显示什么信息（监测参数：多少分钟/多少条；个性化阈值：模型-参数选择）
    // 上述123关系用json写
    // 选择的配置结果也用json写，存储到table的属性中
    // or 不允许新增/删除行，列，只能通过表格设置定义

    var tableCorrelationModel = {
      text: 'Special Table',
      icon: 'table-model',
      onSetup: selectionTargets.onSetupModel,
      onAction: cmd('mceSpecialTable')
    ...
    editor.ui.registry.addMenuItem('specialtable', tableCorrelationModel) // 特定表格
    ...
    // editor.ui.registry.addContextMenu('insertSpecialTable', {
    //   update: function() {
    ...
    //         } else {
    //           return 'specialtable deletetable'
    //         }
    //       }
    //     })
    //   }
  ...
  function Plugin$1() {
    global.add('insertSpecialTable', Plugin)
  }
