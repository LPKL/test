...
(function() {
  ...
  var insertErrorDatetime = function(editor, format) {
    // var $timedom = $('<time>').attr({ 'data-format': format, 'data-type': 'faultTime', 'contenteditable': false }).css({ 'backgroundColor': '#ffff00' }).text(getDateTime(editor, format))
    // var $appendcodeAll = $($timedom)[0].outerHTML
    // editor.execCommand('mceInsertContent', false, $appendcodeAll)
    // console.log('999', $timedom, $appendcodeAll)
    ...
    append_dataorigin.setAttribute('data-type', 'faultTime')
  ...
  var Actions = {
    insertErrorDatetime: insertErrorDatetime,
  ...
  var register$1 = function(editor) {
    ...
    editor.ui.registry.addSplitButton('insertErrorDatetime', {
      icon: 'insert-error-time',
      text: 'Fault Time',
      ...
      onAction: function() {
        ...
        Actions.insertErrorDatetime(editor, defaultFormat.get())
      },
      onItemAction: function(_, value) {
        defaultFormat.set(value)
        Actions.insertErrorDatetime(editor, value)
      }
    })
  }
  ...
  function Plugin() {
    global.add('insertErrorDatetime', function(editor) {
