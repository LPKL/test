...
<script>
import { getAllPipelines, searchPipelines } from '@/api/experiment/pipelines'

import PiplineTable from './components/PiplineTable'
export default {
  name: 'PiplineIndex',
  components: {
    'comp-piplinetable': PiplineTable
  },
  data() {
    return {
      // 故障列表数据
  ...
  methods: {
    ...
    handleFilter(val, p) {
      ...
      const page = p || 1
      this.listLoading = true
      searchPipelines({ search: this.searchstr, page }).then(res => {
        this.listLoading = false
        this.datalist = res.data.data
        this.listQuery.total = res.data.count
        this.listQuery.pagelength = this.datalist.length
    ...
    handleCurrentChange(val) {
      ...
      if (this.searchstr) {
        ...
        this.$router.push({ name: 'PiplineModel', params: { page: val }})
      }
    }
  }
}
