...
export default function WS4Redis(options, $) {
  'use strict'
  var opts, ws, deferred, timer, attempts = 1, must_reconnect = true
  var heartbeat_interval = null, missed_heartbeats = 0

  if (this === undefined) { return new WS4Redis(options, $) }
  if (options.uri === undefined) { throw new Error('No Websocket URI in options') }
  if ($ === undefined) { $ = jQuery }
  ...
  function connect(uri) {
    try {
      ...
      deferred = $.Deferred()
      // ws = new WebSocket(uri, store.getters.session_key) // todo 已经不使用此插件
      ws = new WebSocket(uri, 'store.getters.session_key')
      ws.onopen = on_open
  ...
  function on_open() {
    console.log('Connected!', opts.uri)
  ...
  function on_close(evt) {
    console.log('Connection closed!', opts.uri)
  ...
  function on_error(evt) {
    console.error('Websocket connection is broken!',opts.uri)
